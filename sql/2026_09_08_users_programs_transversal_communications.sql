-- ============================================================================
-- Gestión de Demanda - Comunicaciones transversales sin programa asociado
-- Fecha: 2026-09-08
-- Ajuste:
--   1) Permite users_programs.program_id = NULL para configurar usuarios transversales.
--   2) Mantiene/asegura los flags de comunicaciones agregados al modelo usuario-programa.
--   3) No crea una tabla nueva; se conserva compatibilidad con la estructura actual.
-- ============================================================================

SET @schema_name = DATABASE();

-- Permitir relaciones transversales sin programa asociado.
SELECT IF(
    IS_NULLABLE = 'NO',
    CONCAT('ALTER TABLE users_programs MODIFY COLUMN program_id ', COLUMN_TYPE, ' NULL'),
    'SELECT ''program_id already nullable''' 
) INTO @sql
FROM INFORMATION_SCHEMA.COLUMNS
WHERE TABLE_SCHEMA = @schema_name
  AND TABLE_NAME = 'users_programs'
  AND COLUMN_NAME = 'program_id'
LIMIT 1;

SET @sql = COALESCE(@sql, 'SELECT ''program_id column not found''');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

-- Asegura flags de comunicaciones para instalaciones donde el script anterior no se haya ejecutado.
SET @sql = IF(
...
SET
    can_receive_references = COALESCE(can_receive_references, 0),
...
SELECT
    ...
    user_id,
    program_id,
    CASE WHEN program_id IS NULL THEN 'TRANSVERSAL' ELSE 'PROGRAM' END AS communication_scope,
